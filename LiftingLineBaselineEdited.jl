...
function wing_area(span, chord_root)
total=pi*span/2*chord_root/2
    return total
end
...
function eccentricity(span, chord_root)
    return sqrt(1 - (chord_root / span)^2)
end

# Function to calculate the half-chord length at a given height y
function chordhalf(y, span, ecc)
    return sqrt((span / 2)^2 - y^2) * sqrt(1 - ecc^2)
end
